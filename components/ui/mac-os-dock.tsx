...
interface MacOSDockProps {
  ...
  openApps?: string[];
  className?: string;
  sizeOverride?: { baseIconSize: number; maxScale: number; effectWidth: number };
}
...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const resolved = sizeOverride ?? config;
  const { baseIconSize, maxScale, effectWidth } = resolved;
  const minScale = 1.0;
  const baseSpacing = Math.max(4, baseIconSize * 0.08);

  // Update config on window resize (skipped when sizeOverride is provided)
  useEffect(() => {
  ...
  }, [getResponsiveConfig, sizeOverride]);
    ...
    if (iconRefs.current[index]) {
      if (typeof window !== 'undefined' && (window as unknown as Record<string, unknown>).gsap) {
        const gsap = (window as unknown as Record<string, unknown>).gsap as {
          to: (el: HTMLElement | null, config: Record<string, unknown>) => void;
        };
